...
int WieldClub(){
    if(present("dagger",this_object())){ 
        ...
        new("/domains/etnar/wyr/wyr/weap/dagger_elvish")->eventMove(this_object());
...
static void create() {
    ...
    SetInventory( ([
        "/domains/etnar/wyr/wyr/npc/gwen/dagger_elvish" : "1",
        "/domains/etnar/wyr/wyr/npc/gwen/shield" : "wear shield",
        ] ));
    SetActionsMap( ([
        ]) );
