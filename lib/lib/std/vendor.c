/* Changes made as per Nilrin from
 * The Dead Souls Messaging Board
 * Bug Central: Vendors and Bargaining
 * posted 2009-11-24
 * changes added by ccoker (Lash)
 * 2016-03-04
 */
...
int cmdAppraise(object who, string args){
 ...
 /*     Code shouldn't overide the bargaining skill here.
 ...
 */
 ...
 /* Added the following line so that you can't pay less than value price for an
    item.
 */   
    x+=ob->GetValue(GetLocalCurrency());
...
int GetValue(object ob, object who){
 ...
 /* Switched these two around to reflect who benifits from which side of the
    deal.
    mod = who->GetSkillLevel("bargaining") - GetSkillLevel("bargaining");
 */
