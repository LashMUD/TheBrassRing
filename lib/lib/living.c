    ...
    if( random(sr) > skill ){
        ...
        if( !userp(this_object()) ){
            who->eventPrint("%^RED%^" + GetName() + "%^RED%^ "
                    ...
                    environment(who) );
            this_object()->eventForce("stand"); //bugfix
            this_object()->eventForce("attack " + who->GetKeyName()); //bugfix
            this_object()->SetProperty("steal victim", 1); //is this even used anywhere else?
        }
        return 2;
    }
